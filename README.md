...
5. A dialog will pop up: set the `Roblox Output Folder` line to wherever you want the final output files (FBX and Blender formats) to be saved.
6. Enable the Advanced Settings section if it is disabled, then make sure the `Blender Executable` line is set to the location of your blender executable. Example: `C:\Program Files\Blender Foundation\Blender 3.6\blender.exe`.
7. Set the desired asset name for the output.  This will be the filename of the final output files.
8. Click the Asset Type dropdown and select the desired setting: 
- `Complete Character and Outfit` will produce an R15 Avatar file, an S1 Auto Setup file, and a layered clothing and accessories file.
- `Roblox R15 Avatar` will produce an FBX file that is ready for import using the existing Avatar Import method of Roblox Studio.  
- `Roblox S1 for Avatar Auto Setup` will produce an FBX file for use with the Roblox Avatar Auto Setup feature (currently available as a Beta Feature in Roblox Studio).
- `Layered Clothing or Hair` will produce an FBX file which can be imported and used with the Roblox Studio Accessory Fitting Tool.
- `Rigid Accessories` will produce an FBX file similar to the Layered Clothing option, but without inner and outer cages.
9. There are now multiple optional settings, including multiple `Modesty Overlay` types, customizable `Eyebrows` and `Eyelashes`, and the option to `Bake Single Outfit` to combine all items (inlcuding hair) into a single outfit. If you are exporting a feminine Daz character, it is highly recommended to enable the `Breasts Gone` morph to reduce or remove the breast shape to help comply with Roblox Community Standards.  The `Breasts Gone` morph is part of the Genesis 9 Body Shapes add-on, which is a paid product available on the Daz Store.  If you do not have the Genesis 9 Body Shapes add-on, then you can try manually editing the mesh using Blender or Maya prior to importing into Roblox Studio.  Otherwise, it is recommended that you export only masculine Daz characters in order to comply with Roblox Community Standards.
10. Click the `Accept` button to proceed.  If you see an `Unable to Proceed` button, then may click it for detailed instructions on the missing steps needed to proceed.
11. After clicking the `Accept` button, you will be presented with the DazToRobloxStudio End User License Agreement (EULA).
12. After reading through the EULA, you may click `Accept EULA` to proceed with the conversion process.  Then wait for a dialog popup to notify you when the conversion is complete.
13. If the conversion completed successfully, click `OK` and a window to the Roblox Output Folder should automatically open.
14. If any errors occurred during conversion, you may be able to complete the conversion or find detailed error messages by running a script from the command-line.  Depending on your system setup, an explorer window or filesystem window should popup, showing the DazToRoblox Intermediate Folder.  Open a command-prompt or Terminal window and then run the `manual_blender_script.bat` on Windows or `sh manual_blender_script.sh` on Mac.  This script should start a command-line Blender process and give detailed feedback on any errors that are encountered.
15. If the conversion completed successfully, then an FBX and a Blender file should appear in the Roblox Output Folder.  The filenames should begin with the Asset Name from the Roblox Avatar Exporter window.  The FBX file is ready for import into Roblox Studio.  The Blender file can be opened in Blender to view and edit your character.
16. *If you are using the S1 Avatar Auto Setup pathway, please skip ahead to Step 19.*  If you exported your character from Daz Studio using the `Roblox R15 Avatar` setting, then open Roblox Studio, click the Avatar tab, then click `Import 3D File`.  Select the final FBX output file to import, then click `Open`.  The filename should end with `_R15_ready_to_import.fbx`.
17. In the Import Preview window, click the `Rig General` section.  The Rig Type should be automatically set to `R15` and does not need to be changed.  Click the Rig Scale dropdown and select `Rthro Narrow`.  This will scale legacy Roblox accessories to be more compatible with realistic human body types like Daz characters.  You may try `Rthro` for masculine characters. NOTE: When uploading to Roblox with `Rthro Narrow` Rig Scale, there may be gaps between body parts. You must adjust your avatar's body build by going to Avatar -> Customize -> Head & Body -> Build and move the Body Type to the top left corner of the Body Type triangle.
18. Then click `Import` and you are done.  TIP: Be sure to use appropriate Realistic or Rthro animations instead of the default Roblox animations.  If your Roblox character appears to be leaning back unnaturally when idle, then you need to customize your character animations and select `Realistic-Idle` or `Rthro Idle`.
19. *S1 Avatar Auto Setup:* If you exported your character from Daz Studio using the `Roblox S1 for Avatar Auto Setup` setting, then Open Roblox Studio.  If you have not enabled the Beta Feature for `Avatar Auto Setup` yet, then you can click File -> Beta Features, then scroll down to find `Avatar Auto-Setup Beta` and make sure it is enabled then click Save.  You may need to restart Roblox Studio.  Then click the Avatar tab and click `Import 3D File`.  Select the final FBX output file, then click `Open`.  The filename should end with `_S1_for_avatar_autosetup.fbx`.
...
21. When Auto Setup is complete, it should add a second copy of your character into the scene.  However, this new copy should be fully configured and functioning as a Roblox Avatar.  You are now done.
22. *Blender pathway:* Alternatively, open the `.blend` file in the output folder to adjust cages and attachments and make desired modifications to the mesh or materials.  In addition to body segments, cages and attachments, this `.blend` file also contains a high resolution version of the character that can be used to bake normal maps and other operations.  This HD mesh will be set to invisible by default.
23. If using the `.blend` file from the final output folder, use the following settings when exporting to FBX:
...
24. After exporting to an FBX, you can then import the FBX into Roblox Studio.
25. *Clothing and Hair:* Use the Accessory Fitting Tool or a third-party plugin to set up layered and rigid accessories.
...
The `DevKit` folder contains templates and project files for artists and modellers to use to create custom modesty overlays, eyebrows and eyelashes.  There are multiple scripts within the `DevKit/Scripts` folder to assist users in preparing Daz characters for conversion.
